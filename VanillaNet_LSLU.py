...
from LSLU import LearnableSerieslinearUnit
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, act_num=3, stride=2, deploy=False, ada_pool=None):
        ...
        self.act = LearnableSerieslinearUnit(dim_out, act_num, deploy=self.deploy)
    ...
    def __init__(self,
        ...
        if self.deploy:
            self.stem = nn.Sequential(
                ...
                LearnableSerieslinearUnit(dims[0], act_num, deploy=self.deploy)
            )
        else:
            ...
            self.stem2 = nn.Sequential(
                ...
                nn.BatchNorm2d(dims[0], eps=1e-6),
                LearnableSerieslinearUnit(dims[0], act_num)
            )
...
def vanillanet_5(pretrained=True, in_22k=False, **kwargs):
    model = VanillaNet(dims=[128 * 4, 256 * 4, 512 * 4, 1024 * 4], strides=[2, 2, 2], **kwargs)  # 512, 1024, 2048, 4096
    return model
